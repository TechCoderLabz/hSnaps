...
export function DbuzzFeed() {
  const { isAuthenticated } = useAuthData()
  const { posts, loading, error, hasMore, fetchFeed, loadMore } = useDbuzzStore()
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-zinc-100">DBuzz</h1>
      </div>
      {isAuthenticated && (
        <ComposeFab feedType="dbuzz" placeholder="Drop a buzz..." />
      )}
      {error && (
      ...
      {loading && filteredPosts.length === 0 ? (
        ...
        <EmptyState title="No buzz yet" description="Drop a buzz." />
      ) : (
        <>
          {renderItems(
